...
def build_svg(house_plan: dict, floor_index: int = 0) -> str:
    """Build an SVG string for one floor of a house_plan schema."""
    ...
    parts = [
        ...
        f'text-anchor="middle" fill="#2C3E50">One-Story House Plan</text>',
    ]
    ...
    for room in rooms:
        ...
        label = room.get("name", rtype).replace("_", " ")
...
def main():
    if len(sys.argv) >= 2:
        ...
        house_plan = SAMPLE_HOUSE_PLAN

    out_path = sys.argv[2] if len(sys.argv) >= 3 else "house_plan.svg"
